...
#define MQTT_USERNAME "uMjzpjCqjXe9Q4FVSx7x"
#define MQTT_PASSWORD "123456"
#define MQTT_NAME "PICO_W"
#define MQTT_TOPIC "v1/devices/me/telemetry"

// MQTT Publish Intervals
const unsigned long interval_1 = 60000*5;               // 30 seconds
const unsigned long interval_2 = 12 * 6 * 60 * 1000;  // 12 hours

// Rain Bucket Configurations
#define RAIN_PIN 17
#define DEBOUNCE_TIME 600  // time * 1000 in microseconds required to get through bounce noise
...
// JSON Data Variables
String dataJS_1, dataJS_2, dataJS_3;
char json_1[300], json_2[300], json_3[100];
StaticJsonDocument<200> jsonDocument_1, jsonDocument_2, jsonDocument_3;
char jsonString_1[200], jsonString_2[200], jsonString_3[200];

// DHT Sensor
float Temperature_S, Humidity_S;
...
void loop() {
  ...
  // Publish data at defined intervals
  ...
  if (currentMillis_ - previousMillis_ >= interval_1) {
    ...
    mqtt.publish(MQTT_TOPIC, json_1);
    resetSoilTempSamples();

    // UV data
    jsonDocument_1["UV value"] = mapUV1;
    serializeJson(jsonDocument_1, jsonString_1);
    mqtt.publish(MQTT_TOPIC, jsonString_1);
    ...
    mqtt.publish(MQTT_TOPIC, json_2);
    ...
    mqtt.publish(MQTT_TOPIC, jsonString_2);
    Count = 0;

    // Rain data
    jsonDocument_3["Rainfall per Min"] = rainTrigger;
    serializeJson(jsonDocument_3, jsonString_3);
    mqtt.publish(MQTT_TOPIC, jsonString_3);
...
void reconnectRPC() {
  while (!mqtt.connected()) {
    ...
    if (mqtt.connect("RaspberryPiPicoClient", MQTT_USERNAME, MQTT_PASSWORD)) {
      Serial.println("connected");
...
void MCP3424_scan_initial() {
  ...
  Wire.requestFrom(address_mcp3424_2, (uint8_t)1);
  if (!Wire.available()) {
    Serial.print("No device found at address ");
  ...
  Wire.requestFrom(address_mcp3424_3, (uint8_t)1);
  if (!Wire.available()) {
    Serial.print("No device found at address ");
...
void initial_wind() {
  ...
  attachInterrupt(digitalPinToInterrupt(pinInterrupt), wind_Speed, RISING);
}
...
void Rain_loop() {
  if (timer_rain - nextCalc_rain >= 1000) {
    rainfallPerMinute = rainTrigger * litersPerTip;
    nextCalc_rain = timer_rain;
   // rainTrigger = 0;
  }
}
...
void rainIRQ() {
  ...
  if (currentTime - last_micros_rg >= DEBOUNCE_TIME) {
    rainTrigger++;
    last_micros_rg = currentTime;
  }
}
...
void resetSoilTempSamples() {
  tempSampleSum_0 = tempSampleCount_0 = 0;
  tempSampleSum_1 = tempSampleCount_1 = 0;
  tempSampleSum_2 = tempSampleCount_2 = 0;
  tempSampleSum_3 = tempSampleCount_3 = 0;
}
